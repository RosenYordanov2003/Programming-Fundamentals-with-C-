...
namespace Teamwork_Projects
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            List<Team>validTeams=teams.Where(team=>team.TeamMembers.Count>0).OrderByDescending(team=>team.TeamMembers.Count).ThenBy(team=>team.TeamName).ToList();
            List<Team> disbandsTeams = teams.Where(team => team.TeamMembers.Count == 0).OrderBy(team => team.TeamName).ToList();
